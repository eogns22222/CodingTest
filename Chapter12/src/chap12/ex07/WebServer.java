			...
			Runnable run = new Runnable() {
				...
				public void run() {
					...
					try {
						...
						dis.read(buf);
						// 받아온 byte 를 문자열화 해서 출력
						String request = new String(buf);
						System.out.println(request);
						
						// 요청에 대한 응답(응답이 없으면 대답할 때 까지 클라이언트(브라우저)는 대기한다.)
						// 뭔가를 직접 기록하는 보조 스트림
						...
						String msg = 
								...
								+ "<li style='font-size:25px;'>database</li>"
								+ "<li style='font-size:30px;'>front</li>"
								+ "<li style='font-size:35px;'>jsp</li>"
								+ "<li style='font-size:40px;'>spring</li>"
								+ "<li style='font-size:45px;'>AWS/DOCKER</li>"
								+ "</ul>";
						ps.print(msg);
						ps.flush();
						// 자원반납
